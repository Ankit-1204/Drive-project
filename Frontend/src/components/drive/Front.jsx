...
const Front=()=>{
    ...
    <div>
       <div className="w-full mx-auto px-9 py-1 bg-gray-200">
        ...
        {folder && childFolders && <div className="grid grid-cols-1 md:grid-cols-3 py-5 px-5 gap-6 ">
            {childFolders.map((folder)=>(<div key={folder.key}><Link key={folder.key} to={'/folder/'+(folder.key)} ><button className=" px-6 py-3.5 text-base font-medium text-white inline-flex items-center bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"><IoFolderOpen className=" mr-3 w-5 h-5 md:w-9 md:h-9" /> {folder.name}</button></Link></div>))}
            {childFiles.map((file)=>(<div key={file.key}><button onClick={()=>openFile(file)} className=" px-6 py-3.5 text-base font-medium text-white inline-flex items-center bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 rounded-lg text-center dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800"><FaFile className=" mr-3 w-5 h-5 md:w-9 md:h-9" /> {file.name}</button></div>))}
        </div>}
